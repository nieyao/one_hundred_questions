...
22. hash 路由和 browser 路由的优缺点？

    > hash:

        优点：1.实现简单，兼容性好（兼容到 ie8）。
              2. 绝大多数前端框架均提供了 hash 路由实现。
              3. 不需要服务器端进行任何设置和开发。
              4. 除了资源加载和 Ajax 请求不会发起其他请求。
        缺点：1. 对于部分需要重定向的操作，后端无法获得 hash 部分内容，如 oauth 验证。
              2. 服务器端无法准确跟踪前端路由信息。
              3. 对于需要锚点的功能需求会与路由机制冲突。

    > browser:

        优点：1. 重定向不会丢失 url 的参数，后端可以拿到。
              2. 后端可以准确跟踪路由信息。
